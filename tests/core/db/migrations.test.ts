...
describe("Migrations", () => {
  ...
  describe("runMigrations", () => {
    ...
    it("should create FTS5 virtual table", () => {
      ...
      expect(tables.some((t) => t.name.includes("observations_fts"))).toBe(true);
    });
  ...
  describe("FTS triggers", () => {
    ...
    it("should sync inserts to FTS", () => {
      ...
      // Search FTS
      const results = db
        .prepare("SELECT content FROM observations_fts WHERE content MATCH 'hello'")
    ...
    it("should sync deletes to FTS", () => {
      ...
      // Verify it exists
      let results = db
        .prepare("SELECT content FROM observations_fts WHERE content MATCH 'delete'")
        .all() as { content: string }[];
      expect(results.length).toBe(1);
    ...
    it("should sync updates to FTS", () => {
      ...
      // Update
      db.exec("UPDATE observations SET content = 'updated content' WHERE id = 'o1'");

      // Old content should not match
      let results = db
        .prepare("SELECT content FROM observations_fts WHERE content MATCH 'original'")
        .all() as { content: string }[];
      expect(results.length).toBe(0);

      // New content should match
      results = db
        .prepare("SELECT content FROM observations_fts WHERE content MATCH 'updated'")
        .all() as { content: string }[];
      expect(results.length).toBe(1);
    });
  });
